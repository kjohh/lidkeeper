...
選單列上的一個開關：讓 MacBook 闔蓋後保持清醒。

macOS 沒有提供這個開關。系統設定「電池」裡那個「在使用電源轉接器時，避免在顯示器關閉時自動進入睡眠」只管閒置睡眠，管不到闔蓋；闔蓋是另一條路徑，除非接了外接螢幕（clamshell 模式），否則插著電也一樣會睡。LidKeeper 透過 `pmset disablesleep` 控制睡眠，並提供圖形介面操作。
...
點開面板，就能切換「闔蓋保持清醒」、設定開機自動啟動，或結束 app。

打開 app 之後會從圖示滑出一個小提示告訴你它在哪，幾秒後自己消失。

在咖啡杯狀態下按結束時，會先詢問要保持清醒，還是恢復正常睡眠。這個設定會保留在系統中，結束 app 後仍然生效，但選單列上的開關會隨之消失。
...
第一次按下「闔蓋保持清醒」時，會跳出系統的密碼對話框。完成首次授權後，日常切換不需再次輸入密碼：授權時會在 `/etc/sudoers.d/lidkeeper` 放一條規則，讓你的帳號可以免密碼跑 `pmset -a disablesleep 0` 和 `1` 這兩條指令，其他 `pmset` 參數一律不受影響。
...
不需要任何 Apple 開發者帳號或憑證。在另一台 Mac 下載這個專案，再執行一次上面的三行指令就好。
...
Amphetamine（App Store 免費）功能一樣而且更多，適合需要更多功能的人。這個 app 存在的理由是它只做一件事、程式碼精簡、想改什麼直接改。
